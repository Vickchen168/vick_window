...
class Window(ThemedTk):
    ...
    def click1(self):
        ...
        message = "\n".join(message_data)
        print(message)
        ShowInfo(parent=self,title="全台AQI TOP 5區域",message=message)
    ...
    def click2(self):
        ...
        message = "\n".join(message_data)
        print(message)
        ShowInfo(parent=self,title="全台AQI最差的5區域",message=message)
...
class ShowInfo(Dialog):
    def __init__(self,parent:Misc,title:str|None = None,message:str=""):
        self.message = message ##一定要先super()執行
        super().__init__(parent=parent,title=title)  #最後才執行
        

    
    def body(self, master: Frame) -> Misc | None:
        text = tk.Text(self,height=8,font=('Helvetica',12),width=55)
    ...
    def buttonbox(self) -> None:
        ...
        '''
        box = tk.Frame(self)
        self.ok_button = tk.Button(box, text="確定",fg='red', width=10, command=self.ok)
